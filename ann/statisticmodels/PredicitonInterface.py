...
from statisticmodels.abstract_model import AbstractModel
    ...
    def predict(self, timestamp_start: pd.Timestamp, timestamp_end: pd.Timestamp, interval: int) -> DataFrame:
        ...
        for filename in os.listdir(path):
            if filename.endswith(".pkl"):
                ...
                merged_df = pd.merge(
                    merged_df, prediction[['ds', symbol]], on='ds', how='left')
    ...
    def predict(self, timestamp_start: pd.Timestamp, timestamp_end: pd.Timestamp, interval: int) -> DataFrame:
        ...
        for filename in os.listdir(path):
            if filename.endswith(".pkl"):
                ...
                prediction.rename(
                    columns={'HistoricAverage': symbol}, inplace=True)
                prediction['ds'] = business_days_series
    ...
    def predict(self, timestamp_start: pd.Timestamp, timestamp_end: pd.Timestamp, interval: int) -> DataFrame:
        ...
        for filename in os.listdir(path):
            if filename.endswith(".pkl"):
                ...
                prediction.rename(
                    columns={'WindowAverage': symbol}, inplace=True)
                prediction['ds'] = business_days_series
