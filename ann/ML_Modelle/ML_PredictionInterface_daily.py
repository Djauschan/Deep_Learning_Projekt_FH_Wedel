...
class ABC_LinearRegressionModel_daily(AbstractModel):
    ...
    def load_data(self, symbol):
        file_path = 'ann/ML_Modelle/saved_pkl_model_daily/Data'
        data = {}
    ...
    def load_model(self, symbol) -> None:
        model_path = f'ann/ML_Modelle/saved_pkl_model_daily/LR-Model/{symbol}_lr_model.pkl'
...
class ABC_RandomForestModel_daily(AbstractModel):
    ...
    def load_model(self, symbol) -> None:
        model_path = f'ann/ML_Modelle/saved_pkl_model_daily/RF-Model/{symbol}_rf_model.pkl'
        if os.path.exists(model_path):
            with open(model_path, 'rb') as file:
                self.model = pickle.load(file)
        else:
...
class ABC_GradientBoostingModel_daily(AbstractModel):
    ...
    def load_model(self, symbol) -> None:
        model_path = f'ann/ML_Modelle/saved_pkl_model_daily/GBM-Model/{symbol}_gbm_model.pkl'
        if os.path.exists(model_path):
            with open(model_path, 'rb') as file:
                self.model = pickle.load(file)
        else:
...
class ABC_SVMModel_daily(AbstractModel):
    ...
    def load_model(self, symbol) -> None:
        model_path = f'ann/ML_Modelle/saved_pkl_model_daily/SVM-Model/{symbol}_svm_model.pkl'
        if os.path.exists(model_path):
            with open(model_path, 'rb') as file:
                self.model = pickle.load(file)
        else:
