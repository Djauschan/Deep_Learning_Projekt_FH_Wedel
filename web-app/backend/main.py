...
async def update_budget_by_user(
    username: str, budgetInput: int, db: Session = Depends(get_db)
):
...
def calculate_end_date(start_date: str, resolution: str):
    if resolution == "D":
        ...
        return (pd.Timestamp(start_date) + pd.Timedelta(minutes=19)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    else:
...
def predict_randomForest(
    stock_symbols: str = "[AAPL]", start_date: str = "2021-01-04", resolution: str = "H"
):
    if resolution == "M":
        start_date += " 10:01:00"
    end_date = calculate_end_date(start_date, resolution)
...
def load_data(
    ...
    if resolution == "H":
        end_date = (pd.Timestamp(start_date) + pd.Timedelta(days=2)).strftime(
            "%Y-%m-%d"
        )
    else:
        end_date = calculate_end_date(start_date, resolution)

    data = crud.loadDataFromFile(
        stock_symbols=stock_symbols,
...
@app.get("/get/MAE")
def get_MAE_for_model(
    stock_symbols: str = "[AAPL, SNAP]",
    start_date: str = "2021-01-04",
    resolution: str = "H",
    ...
    for stock_symbol in stock_symbols:
        try:
            ...
            return_dict[stock_symbol] = {
                "MAE": mean_absolute_error(
                    df_complete["value_pred"].values, df_complete["Close"].values
                ),
                "ME": mean_error(
                    df_complete["value_pred"].values, df_complete["Close"].values
                ),
                "profit": get_model_profit(
                    df_complete["value_pred"].values, df_complete["Close"].values
                ),
            }
        except Exception as e:
            print(f"Error: {e}")
            return_dict[stock_symbol] = {"MAE": -999.9, "ME": -999.9, "profit": -999.9}
