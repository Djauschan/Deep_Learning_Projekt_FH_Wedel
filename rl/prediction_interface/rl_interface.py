...
class RLInterface:
    ...
    def __init__(self) -> None:
        ...
        models = [
            QLearningPredictor(model)
            for model in glob("prediction_interface/api_models/*.npy")
        ]
        self.models = {
            model.name: model for model in models if model.name != "unsupported"
        }
    ...
    def predict(
        ...
        if resolution == resolution_type.DAILY:
            time_range = pd.date_range(
                f"{start_date} 20:00:00", f"{end_date} 20:00:00", freq="D"
            )
            resolution = "D"
        ...
        for current_date in time_range:
            if (
                current_date not in relevant_data.index
                and resolution_type(resolution) != resolution_type.DAILY
            ):
                continue
            if last_timestamp is not None:
                ...
                for model_name, model in self.models.items():
                    if model_name != "ensemble" and "ML" not in model_name:
                        ...
                        predictions[current_date][model_name] = self.action_map[
                            current_prediction
                        ]
                        ...
                        ensemble_input.append(current_prediction)
                        predictions[current_date][model_name.split("-")[0]] = (
                            self.action_map[current_prediction]
                        )

                if self.ensemble_mode:
                    predictions[current_date]["ensemble"] = self.action_map[
                        self.models["ensemble"].predict(ensemble_input)
                    ]
                else:
    ...
    def _read_data(self) -> dict[str, pd.DataFrame]:
        ...
        for filepath in glob("../data/Aktien/*.txt"):
            df = pd.read_csv(
                ...
                index_col=0,
                parse_dates=True,
                names=["DateTime", "Open", "Close", "High", "Low", "_"],
            ).drop("_", axis=1)
    ...
    def _get_data_from_api(
        self, url: str, stock_symbol: str, start_date: pd.Timestamp, resolution
    ) -> pd.DataFrame:
        ...
        if cache_parameter not in self.response.keys():
            self.response[cache_parameter] = requests.get(
                url,
                params={
                    "stock_symbols": f"[{stock_symbol.upper()}]",
    ...
    def _get_prediction_from_dict_list(
        self, dict_list: list[dict[str:str]], desired_date: pd.Timestamp
    ) -> float:
        ...
        try:
            return (
                pd.DataFrame(
                    dict_list,
                    index=[
                        pd.Timestamp(dictionary["date"]) for dictionary in dict_list
                    ],
                )
                .loc[desired_date]
                .value
