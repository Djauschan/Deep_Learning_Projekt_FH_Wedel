...
def generate_data(start_date, end_date, interval_minutes):
    ...
    while current_date <= end_date:
        if 4 <= current_date.hour < 20:  # Ensure time is between 4 am and 8 pm
            value = 30 if current_date <= start_date + timedelta(days=2) else round(slope * (
                current_date - (start_date + timedelta(days=2))).total_seconds() / (60 * interval_minutes)) + 30
...
if __name__ == "__main__":
    ...
    save_to_txt(
        generated_data,
        "../../data/input/us3000_tickers_A-B_1min_iqksn/art2_1min.txt")
